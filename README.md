...
- Singleton: `DriverManager` owns a single Appium session per test JVM.
- Factory: `DriverFactory` creates the Android driver and capabilities.
- Page Object: screens are modeled under `src/main/java/com/homechallenge/pages`.
- Test lifecycle extension/hooks: the driver is closed explicitly at suite end.
- Evidence and diagnostics: Allure attachments include final screenshots, animated screen recordings, page source, device dumps, process list, and first failing project line when a test fails.

## APK Analysis

- Expected local APK path in the project root: `app-home-test-mobile.apk`
- SHA-256: `F0C27753CEEE2E7BEED9C3F28760BB94BE4282FABDE5E02902B578D4D7A52A7E`
- Android package: `com.learnautomationapp`
- Launch activity: `.MainActivity` when using `appPackage`/`appActivity`
- Main stable locators found in the React Native bundle:
  - Login: `emailField`, `passwordField`, `loginButton`, `registerButton`
  - Catalog: `itemsList`, `itemName`, `itemImage`
  - Details: `title`, `author`, `date`, `textDescription`
  - Registration: `firstNameField`, `lastNameField`, `continueButton`, `addressInput`, `cityInput`, `zipInput`, `openDatePicker`, `termConditions`, `signupButton`, `goLoginButton`
...
The Android container installs `app-home-test-mobile.apk` once during its health check. The test containers then use `SKIP_APP_INSTALL=true` and start sessions with `appPackage=com.learnautomationapp` plus `appActivity=.MainActivity`, so Appium does not reinstall the large APK for every scenario.
